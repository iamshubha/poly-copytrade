...
const postHandler = withAuth(async (req: NextRequest, userId: string) => {
  ...
  try {
    ...
    if (!validation.valid) {
      log.warn('Invalid trade parameters', { error: validation.error });
      return errorResponse(validation.error || 'Invalid trade parameters', 400);
    }
    ...
    log.logTrade('trade-executed', trade);
