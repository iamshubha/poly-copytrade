...
export async function GET(req: NextRequest) {
  try {
    ...
    console.log("[Markets API] Filters:", { limit, active, search, category, sortBy, sortOrder, offset });
    ...
    console.log(`[Markets API] Fetched ${Array.isArray(markets) ? markets.length : 0} markets from API`);
    
    // Filter to show only markets that are NOT closed (active trading markets)
    markets = Array.isArray(markets) ? markets.filter((market: any) => !market.closed) : [];
    console.log(`[Markets API] After filtering out closed markets: ${markets.length} markets`);
    ...
    markets = markets.map((m: any) => {
      ...
      const volume = typeof m.volumeNum === 'number' ? m.volumeNum : 
    ...
    console.log('[Markets API] Sample market data:', markets.length > 0 ? {
    ...
    if (search && markets.length > 0) {
      ...
      markets = markets.filter(
        (market) =>
          ...
          (market.description?.toLowerCase().includes(searchLower) || false)
      );
      console.log(`[Markets API] After search filter: ${markets.length} markets`);
    }
    ...
    if (category && category !== "all" && markets.length > 0) {
      ...
      console.log(`[Markets API] After category filter: ${markets.length} markets`);
    }
    ...
    const transformedMarkets = markets.map((market) => {
      ...
      if (market.outcomePrices) {
        try {
          ...
          console.warn('[Markets API] Failed to parse outcomePrices:', market.outcomePrices);
        }
      }
